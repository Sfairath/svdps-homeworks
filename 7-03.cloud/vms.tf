...
#Задание 3. Создаём машину для базы данных
resource "yandex_compute_instance" "db" {
  ...
  network_interface {
    subnet_id          = yandex_vpc_subnet.develop_a.id #Подсеть в зоне а
    nat                = false
    security_group_ids = [yandex_vpc_security_group.LAN.id, yandex_vpc_security_group.web_sg.id] #Разрешаем только подключения внутри сети
